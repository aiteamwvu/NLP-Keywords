...
def getKeywords(text, numOfKeywords, title=""):
    ...
    def scrubList(words):
        return list( [ word.encode('ascii', 'replace') for word in words])
...
def getLabelsAndKeys(url, keys=20):
	art = Article(url, language='en')
	art.download()
	art.parse()
	keys = getKeywords(art.text, keys)
	return { 'title' : art.title, 'keys' : keys}

if __name__ == '__main__':
	url = 'https://techcrunch.com/2017/10/06/apple-is-looking-into-reports-of-iphone-8-batteries-swelling/'
	print(getLabelsAndKeys(url))
